...
// Function to find Python executable - looks for portable Python first
function findPythonExecutable() {
  const appPath = process.execPath; // Path to the executable
  const appDir = path.dirname(appPath);
  
  // Check for portable Python in various locations relative to the executable
  const portablePaths = [
    path.join(appDir, 'WinPython', 'python.exe'),
    path.join(appDir, '..', 'WinPython', 'python.exe'),
    path.join(appDir, 'python', 'python.exe'),
    path.join(appDir, '..', 'python', 'python.exe')
  ];
  
  // Check each portable path
  ...
  console.log('No portable Python found, using system Python');
  return 'python';
}

// Function to find FFmpeg executable - looks for bundled FFmpeg first
function findFFmpegExecutable() {
  try {
    // Use app.asar.unpacked path for FFmpeg files
    const ffmpegPath = path.join(__dirname.replace('app.asar', 'app.asar.unpacked'), 'ffmpeg', 'ffmpeg.exe');
    
    if (fs.existsSync(ffmpegPath)) {
      console.log('Found bundled FFmpeg at:', ffmpegPath);
      return ffmpegPath;
    }
  } catch (error) {
    console.log('Error checking for bundled FFmpeg:', error.message);
  }
  
  // Fall back to system FFmpeg
  console.log('No bundled FFmpeg found, using system FFmpeg');
  return 'ffmpeg';
}
...
ipcMain.handle('start-recording', async () => {
  ...
  try {
    ...
    // Spawn FFmpeg process using bundled executable
...
async function transcribeAudioFile(audioFilePath) {
  ...
  try {
    // Use app.asar.unpacked path for Python files
    const pythonScript = path.join(__dirname.replace('app.asar', 'app.asar.unpacked'), 'python', 'transcribe.py');
    
    // Spawn Python process with file input
    const pythonExecutable = findPythonExecutable();
    const python = spawn(pythonExecutable, [pythonScript, '--input', audioFilePath, '--model', 'base'], {
      stdio: ['pipe', 'pipe', 'pipe']
    });
...
ipcMain.handle('transcribe-audio', async (event, audioData) => {
  ...
  try {
    ...
    const pythonExecutable = findPythonExecutable();
    const python = spawn(pythonExecutable, [pythonScript, '--stdin'], {
      stdio: ['pipe', 'pipe', 'pipe']
    });
    ...
    return new Promise((resolve, reject) => {
      ...
      python.on('close', (code) => {
        ...
        // Try to parse the result regardless of exit code, as Python script may output valid JSON
        try {
          const result = JSON.parse(stdout);
          if (result.success) {
            resolve(result.text);
          ...
          // Only if we can't parse the JSON, check the exit code
          if (code !== 0) {
            reject(new Error(`Python process failed with code ${code}: ${stderr}`));
          } else {
            reject(new Error('Failed to parse transcription result'));
...
ipcMain.handle('translate-text', async (event, text, targetLanguage) => {
  ...
  try {
    const { spawn } = require('child_process');
    // Use app.asar.unpacked path for Python files
    const pythonScript = path.join(__dirname.replace('app.asar', 'app.asar.unpacked'), 'python', 'translate.py');
...
ipcMain.handle('export-settings', async () => {
  try {
    const result = await dialog.showSaveDialog(settingsWindow || mainWindow, {
      ...
      defaultPath: 'hearai-settings.json',
      filters: [
